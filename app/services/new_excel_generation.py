...
class ExcelGenerationService:
    ...
    def generate_quotation_excel(self, gmail_id: str, extraction_data: Dict, copy_only: bool = False) -> Optional[str]:
        ...
        try:
            ...
            logger.info(f"Extraction result type: {type(extraction_result)}, keys: {list(extraction_result.keys()) if isinstance(extraction_result, dict) else 'Not a dict'}")
            if extraction_result:
                ...
                if not win32com_success:
                    logger.info(f"win32com not successful (win32com_success={win32com_success}), using openpyxl fallback")
                    try:
                        from openpyxl import load_workbook
                        
                        logger.info("Using openpyxl to fill template (Mac/Linux compatible)")
                        logger.info(f"Extraction result for openpyxl: {json.dumps(extraction_result, indent=2)[:500]}")
                        ...
                        # Fill the template
                        self._fill_quotation_template_openpyxl(ws, extraction_data, extraction_result)
                        
                        # Save the workbook
                        wb.save(output_path)
                        wb.close()
                        
                        logger.info("Template filled using openpyxl - data inserted successfully")
    ...
    def _fill_quotation_template_win32(self, ws, email_data, extraction_result):
        try:
            ...
            try:
                meta_to = extraction_result.get("to", "") or (email_data.get("extraction_result", {}) or {}).get("to", "")
                meta_email = extraction_result.get("email", "") or (email_data.get("extraction_result", {}) or {}).get("email", "")
                meta_mobile = extraction_result.get("mobile", "") or (email_data.get("extraction_result", {}) or {}).get("mobile", "")
                meta_subject = email_data.get("subject", "") or extraction_result.get("subject", "")
            ...
            for idx, item in enumerate(requirements[:MAX_ROWS]):
                ...
                cell.GetCharacters(len(text1)+len(text2)+1, len(text3)).Font.Color = 0x0000FF
                cell.GetCharacters(len(text1)+len(text2)+1, len(text3)).Font.Bold = True
    ...
    def _fill_quotation_template_openpyxl(self, ws, email_data, extraction_result):
        try:
            ...
            # --- Header: write email metadata into first row (openpyxl) ---
            try:
                ...
                ws.cell(row=1, column=1).value = f"To: {meta_to}"
                ws.cell(row=1, column=2).value = f"Email: {meta_email}"
                ws.cell(row=2, column=1).value = f"Mobile: {meta_mobile}"
                ws.cell(row=2, column=2).value = f"Subject: {meta_subject}"
            ...
            for idx, item in enumerate(requirements[:MAX_ROWS]):
                ...
                # Use formula so Excel recalculates when values change
            ...
            ws.cell(row=TOTAL_ROW, column=6).value = f"=SUM(F{sum_start}:F{sum_end})"
            ws.cell(row=TOTAL_ROW, column=6).number_format = '#,##0.00'
            ws.cell(row=VAT_ROW, column=6).value = f"=F{TOTAL_ROW}*0.05"
            ws.cell(row=VAT_ROW, column=6).number_format = '#,##0.00'
            ws.cell(row=GRAND_ROW, column=6).value = f"=F{TOTAL_ROW}*(1+0.05)"
